...
class RPN(nn.Cell):
    ...
    def _make_rpn_layer(self, num_layers, in_channels, feat_channels, num_anchors, cls_out_channels):
        ...
        for i in range(num_layers):
            ...
            if self.device_type == "Ascend":
                rpn_reg_cls_block.to_float(mstype.float32)
    ...
    def construct(self, inputs, img_metas, anchor_list, gt_bboxes, gt_labels, gt_valids, selected_id):
        "RPN"
        loss_print = ()
        rpn_cls_score = ()
        rpn_bbox_pred = ()
        ...
        if self.training:
            ...
            for i in range(self.num_layers):
                ...
                #########loss_mask#########
                step_size = cls_score_i.shape[0] // self.batch_size
                loss_mask = Tensor(np.zeros((self.feature_anchor_shape[i])), dtype=self.ms_type)
                ...
                loss_reg = loss_reg * self.tile(self.reshape(loss_mask, (-1,1)), (1,4))
