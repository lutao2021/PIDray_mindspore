...
class DenseNoTranpose(nn.Cell):
    ...
    def construct(self, x):
        if self.device_type == "Ascend":
            ...
            weight = self.cast(self.weight, mstype.float32)
...
class Rcnn_2(nn.Cell):
    ...
    def construct(self, featuremap, bbox_targets, labels, mask, selected_id, stage=None):
        "Stage 3 RCNN"
        ...
        if self.training:
            ...
            loss, loss_cls, loss_reg, loss_print = self.loss(x_cls, x_reg, bbox_targets, bbox_weights, labels_, mask, selected_id)
    ...
    def loss(self, cls_score, bbox_pred, bbox_targets, bbox_weights, labels, weights, selected_id):
        """Loss method."""

        #########loss_mask#########
        step_size = labels.shape[0] // self.train_batch_size
        loss_mask = Tensor(np.zeros((self.num_bboxes)), dtype=self.ms_type)
        for id in range(self.train_batch_size):
            if selected_id[id] == 1:
                loss_mask[id * step_size: (id + 1) * step_size] = 1
        #########loss_mask#########

        loss_print = ()
        labels = self.cast(labels, mstype.float32)
        loss_cls, _ = self.loss_cls(cls_score, labels)
        loss_cls = loss_cls * loss_mask
        ...
        loss_reg = loss_reg * self.tile(self.reshape(loss_mask, (-1, 1, 1)), (1, self.num_classes, 4))
